  ...
  journey: {
    ...
    milestones: [
      ...
      },
      {
        ...
        href: "wiki/coding-test/index.html#first-loop",
        goal: "배달·땅따먹기·정수 삼각형의 첫 학습을 마쳤습니다. 이제 등굣길에서 장애물과 격자 경계의 경로 수를 DP 상태·점화식·초깃값으로 연결하고, 앞선 DP 문제는 다음 복습에서 자료 없이 다시 구성합니다.",
        evidence: "정수 삼각형에서 이전 행의 위치별 최고 누적합을 저장하고 왼쪽 끝·가운데·오른쪽 끝의 서로 다른 부모 후보를 직접 설명·구현해 정확성 10개·효율성 10개, 총 20개 테스트를 모두 통과했습니다. 각 칸 계산은 O(1)이지만 행 길이가 1부터 N까지 증가하므로 전체 칸 합과 상한 관점에서 시간 O(N²)이고, ac_value·new_value가 각각 최대 N개라 두 배열을 함께 보유해도 추가 공간 O(N)이라고 설명했습니다. 정수 삼각형 첫 학습은 완료했지만 등굣길과 자료 없는 재구현이 남아 통합 관문은 현재로 유지합니다."
      },
      {
