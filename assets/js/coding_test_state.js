  ...
  journey: {
    ...
    milestones: [
      ...
      },
      {
        ...
        href: "wiki/coding-test/index.html#first-loop",
        goal: "정수 삼각형의 왼쪽 끝·가운데·오른쪽 끝 전이를 구현해 전체 테스트에 통과했습니다. 이제 삼각형 전체 칸 수와 두 행 상태를 기준으로 시간·공간 복잡도를 설명한 뒤 첫 학습을 닫고 등굣길로 이동합니다. 땅따먹기는 다음 복습에서 자료 없이 다시 구성합니다.",
        evidence: "배달과 땅따먹기의 첫 학습을 마친 뒤, 정수 삼각형에서 이전 행의 위치별 최고 누적합을 저장하고 왼쪽 끝은 이전[0], 가운데는 이전[i-1]·이전[i]의 최댓값, 오른쪽 끝은 이전[i-1]을 사용하는 전이를 직접 설명·구현했습니다. i == len(row)를 i == len(row)-1로 고치고 가운데 max, 행 완료 뒤 ac_value 교체, 한 행 입력에서도 안전한 max(ac_value) 반환을 반영해 정확성 10개·효율성 10개, 총 20개 테스트를 모두 통과했습니다. 시간·공간 복잡도 설명은 아직 확인하지 않아 정수 삼각형 첫 학습과 통합 관문은 완료하지 않았습니다."
      },
      {
